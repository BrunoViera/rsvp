...
-- Lectura pública: se necesita para el flujo de "lista" (ver nombres y estado)
-- y para resolver el link personal por token.
create policy "public_select_guests" on public.guests
  for select using (true);

-- GUESTS: un invitado anónimo puede agregarse a sí mismo (flujo de lista, "no encuentro mi nombre")
create policy "public_self_insert_guest" on public.guests
  for insert with check (source = 'self');

-- GUESTS: un invitado anónimo puede actualizar SOLO su propia fila para confirmar/declinar
-- (se filtra por id + rsvp_token desde la app; el check aquí es permisivo a nivel de RLS
-- porque no hay sesión de usuario para el invitado, la protección real es el token en la URL)
create policy "public_update_guest_rsvp" on public.guests
  for update using (true);

-- ------------------------------------------------------------
-- Nota de seguridad:
-- Como el invitado no tiene sesión de Supabase Auth, la tabla guests
-- queda con lectura/actualización pública a nivel de RLS. La protección
-- práctica es que solo quien tiene el link (token o slug del evento)
-- puede ver/editar esos datos. No se guarda información sensible.
-- ------------------------------------------------------------
